...
class TextBoxWithIcons {
            ...
            if (!nextTextBit) {
                ...
                if (result && result.index === 0) {
                    ...
                    if (element.getAttribute('src')) {
                        ...
                        let icon = new PIXI.Sprite(PIXI.Texture.fromImage(
                            element.getAttribute('src')));

                        //icon.y = lines.length*PIXI.TextMetrics.measureText(this.text,this.style).lineHeight;

                        if (lines.last().length > 0) {
                            icon.x = lines.last().last().x +
                                lines.last().last().width; //+margin ?
                        } else {
                            icon.x = 0;
                        }

                        icon.width = this.style.fontSize *
                            element.getAttribute('width');
                        //console.log(icon.width);
                        icon.height = this.style.fontSize *
                ...
                let textView = new PIXI.Text(
                    nextTextBit.replace(/(\r\n|\n|\r)/gm, ''));

                textView.style = this.style;
                //textView.y = lines.length*PIXI.TextMetrics.measureText(this.text,this.style).lineHeight;
                if (lines.last().length > 0) {
                    textView.x = lines.last().last().x +
                        lines.last().last().width; //+char space?
                } else {
                    textView.x = 0;
                }

                textView.width = PIXI.TextMetrics.measureText(nextTextBit,
                    this.style).width;
    ...
    getTextToWriteUntilTagOrEndline(textToWrite, line) {
        if (textToWrite.length > 0) {
            ...
            while (PIXI.TextMetrics.measureText(t, this.style).lines.length <
            2 && PIXI.TextMetrics.measureText(t, this.style).width + start <
            this.box.width - 20 && i < textToWrite.length) {
                i += 1;
